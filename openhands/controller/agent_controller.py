...
class AgentController:
    ...
    async def start_delegate(self, action: AgentDelegateAction) -> None:
        ...
        # The delegate gets its own iteration budget (iteration_delta)
        # rather than sharing the partially-consumed parent flag
        delegate_iteration_flag = IterationControlFlag(
            limit_increase_amount=self._initial_max_iterations,
            current_value=0,
        ...
        if self._initial_max_budget_per_task:
            delegate_budget_flag = BudgetControlFlag(
                limit_increase_amount=self._initial_max_budget_per_task,
                current_value=0,
                max_value=self._initial_max_budget_per_task,
                headless_mode=self.headless_mode,
            )
        ...
        state = State(
            ...
            iteration_flag=delegate_iteration_flag,
            budget_flag=delegate_budget_flag,
    ...
    def end_delegate(self) -> None:
        ...
        # Add the delegate's iteration count to the parent's count
        # since the delegate had its own separate iteration budget
