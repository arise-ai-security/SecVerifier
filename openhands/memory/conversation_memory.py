...
class ConversationMemory:
    ...
    def _ensure_initial_user_message(
        ...
        """Ensure exactly one initial user MessageAction exists and is at index 1.

        Previous logic inserted the initial user message at index 1 on every call when
        events[1] was not a user message, which could lead to duplicate initial messages
        being appended repeatedly across condensation cycles. This implementation:
        - Finds all user MessageAction events matching the provided initial_user_action
          by CONTENT (not by object identity),
        - Removes duplicates, keeping a single occurrence,
        - Ensures that single occurrence is positioned at index 1 (right after the
          SystemMessageAction), inserting it only when necessary without duplicating.
        """
        ...
        # Collect indices of existing initial user messages (match by content)
        existing_indices: list[int] = []
        for idx, ev in enumerate(events):
            if isinstance(ev, MessageAction) and ev.source == 'user':
                if _same_initial(ev, initial_user_action):
                    existing_indices.append(idx)

        # Remove duplicates: keep the first occurrence (if any), delete the rest
        if len(existing_indices) > 1:
            # remove from the end to keep indices valid
            for idx in reversed(existing_indices[1:]):
                del events[idx]
            # After deletions, recompute the first occurrence index (it may have shifted
            # if earlier duplicates were removed). The first kept index is existing_indices[0],
        ...
        # At this point exactly one exists
        current_idx = existing_indices[0]

        # Ensure events[0] is system; if not, we won't reorder aggressively, but we still try
        # to position the initial user message early. Prefer index 1 if possible.
        desired_idx = 1

        if current_idx != desired_idx:
            # Move the existing initial message to index 1 without duplicating
            msg = events.pop(current_idx)
