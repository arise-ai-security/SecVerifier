...
def load_microagents_from_dir(
    ...
    if microagent_dir.exists():
        ...
        if (microagent_dir.parent.parent / '.cursorrules').exists():
            ...
            logger.debug(f'Found .cursorrules file: {cursorrules_files[0]}')

        md_files = [f for f in microagent_dir.rglob('*.md') if f.name != 'README.md']
        logger.debug(f'Found {len(md_files)} .md files (excluding README.md)')

        # Process all files in one loop
        total_files = len(cursorrules_files) + len(md_files)
        ...
        for idx, file in enumerate(chain(cursorrules_files, md_files), 1):
            try:
                ...
                if isinstance(agent, RepoMicroagent):
                    ...
                    logger.debug(f'  -> Added RepoMicroagent: {agent.name}')
                elif isinstance(agent, KnowledgeMicroagent):
                    # Both KnowledgeMicroagent and TaskMicroagent go into knowledge_agents
                    knowledge_agents[agent.name] = agent
                    logger.debug(f'  -> Added KnowledgeMicroagent: {agent.name}')
        ...
        logger.debug(f'Microagent directory does not exist: {microagent_dir}')
