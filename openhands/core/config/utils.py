...
def finalize_config(cfg: OpenHandsConfig) -> None:
    ...
    # Handle the sandbox.volumes parameter and migrate deprecated workspace_* automatically
    ...
    # Handle the deprecated workspace_* parameters by auto-migrating to SANDBOX_VOLUMES
    elif cfg.workspace_base is not None or cfg.workspace_mount_path is not None:
        # Prefer workspace_base if provided, otherwise use workspace_mount_path
        base_path = cfg.workspace_base or cfg.workspace_mount_path
        if base_path is not None:
            base_path = os.path.abspath(base_path)
            # Auto-generate SANDBOX_VOLUMES and normalize related fields
            generated_volume = f"{base_path}:/workspace:rw"
            cfg.sandbox.volumes = generated_volume
            cfg.workspace_base = base_path
            cfg.workspace_mount_path = base_path
            cfg.workspace_mount_path_in_sandbox = '/workspace'
            logger.openhands_logger.info(
                "Detected deprecated WORKSPACE_* settings. Automatically migrated to SANDBOX_VOLUMES='%s'.",
                generated_volume,
            )
        # Apply mount rewrite if explicitly set (back-compat), but avoid warnings
    ...
    if cfg.sandbox.use_host_network and platform.system() == 'Darwin':
        # Keep as informational to avoid noisy warnings on macOS
        logger.openhands_logger.info(
            'Host network mode on macOS requires Docker Desktop 4.29.0+ with host network enabled. '
            'See https://github.com/docker/roadmap/issues/238#issuecomment-2044688144.'
        )
...
def get_llm_config_arg(
    ...
    # If not found, attempt to interpret common shorthand names and
    # build a config based on the default [llm] section.
    # This allows users to pass values like "llm.4o" without defining [llm.4o] explicitly.
    alias_map = {
        '4o': 'gpt-4o',
        'gpt-4o': 'gpt-4o',
        '4o-mini': 'gpt-4o-mini',
    ...
    # Extract default llm settings (non-table keys)
    default_llm_settings: dict[str, object] = {}
    if 'llm' in toml_config:
        for k, v in toml_config['llm'].items():
            if not isinstance(v, dict):
                default_llm_settings[k] = v

    if llm_config_arg in alias_map:
        # Start from defaults to preserve API keys and other settings
        defaults = default_llm_settings.copy()
        defaults['model'] = alias_map[llm_config_arg]
        logger.openhands_logger.info(
            'LLM config "%s" not found in %s. Using default [llm] with model="%s".',
            llm_config_arg,
            toml_file,
            defaults['model'],
        )
